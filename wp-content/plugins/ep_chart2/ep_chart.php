...
function wporg_add_dashboard_widgetss() {
    wp_add_dashboard_widget(
        'wporg_dashboard_widget',                          
        esc_html__( 'chart', 'wporg' ), 
        'wporg_dashboard_widget_renderr'                    
    ); 
}
add_action( 'wp_dashboard_setup', 'wporg_add_dashboard_widgetss' );
function wporg_dashboard_widget_renderr() {
?>
<?php
...
// }
// }
function wpdocs_add_dashboard_widgetss() {
    wp_add_dashboard_widget( 'dashboard_widget', 'api', 'dashboard_widgets_function' );
}
add_action( 'wp_dashboard_setup', 'wpdocs_add_dashboard_widgetss' );
 
function dashboard_widgets_function( $post, $callback_args ) {
    esc_html_e( "", "textdomain" );
    $curl = curl_init();

curl_setopt_array($curl, array(

  CURLOPT_URL => "http://127.0.0.1:8050/_reload-hash",
